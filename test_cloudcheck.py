...
def test_cidr_defragmentation():
    ...
    log = logging.getLogger("test_cloudcheck")
    ...
    # Verify the results
    assert len(defragmented) < len(networks), "Defragmentation should reduce the number of networks"
    ...
    assert actual_networks == expected_networks, f"Expected {expected_networks}, got {actual_networks}"
...
def test_cidr_defragmentation_mixed_ipv4_ipv6():
    ...
    defrag_ipv4_count = sum(1 for n in defragmented if isinstance(n, ipaddress.IPv4Network))
    defrag_ipv6_count = sum(1 for n in defragmented if isinstance(n, ipaddress.IPv6Network))
    assert defrag_ipv4_count > 0, "Should still have IPv4 networks after defragmentation"
    assert defrag_ipv6_count > 0, "Should still have IPv6 networks after defragmentation"
